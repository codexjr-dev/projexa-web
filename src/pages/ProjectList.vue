          ...
          <template v-slot="scope">
            <div class="actions">
              <div
                ...
                v-if="isLeadership || isOnTeam(scope.row)"
    ...
    >
      ...
      <template v-slot:footer>
        <span class="dialog-footer">
          <el-button
            v-if="!isVisualizar"
            @click="isEditar ? editProject() : saveProject()"
            type="primary"
            color="#4b53c6"
          >
            Salvar
...
export default {
  ...
  methods: {
    ...
    getTeamMembersId(row) {
      return row.team[0] && row.team[0].name
        ? row.team.map((member) => member._id)
        : row.team;
    },
    ...
    async saveNews() {
      try {
        await this.createNews({
          news: this.newsToBeCreated,
          projectId: this.novoProjeto._id,
        });
